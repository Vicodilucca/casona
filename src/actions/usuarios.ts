...
export async function crearUsuario(nombre: string, email: string, rol: Rol) {
  ...
  const creado = await crearUsuarioDb({ nombre: n, email: e, passwordHash: hash, rol });

  await logAudit(actor, 'crear', 'usuario', creado.id, { email: e, rol });
  revalidatePath('/usuarios');

  return { id: creado.id, tempPassword };
}

export async function editarUsuario(id: number, nombre: string, email: string, rol: Rol) {
  const actor = await requireAuth('superadmin');
  const { nombre: n, email: e } = validarNombreEmail(nombre, email);
  ...
  await editarUsuarioDb(id, { nombre: n, email: e, rol });
  await logAudit(actor, 'editar', 'usuario', id, { email: e, rol });
  revalidatePath('/usuarios');
}
