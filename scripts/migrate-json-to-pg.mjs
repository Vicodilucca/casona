...
const dataPath = join(__dirname, '..', 'quinta-data.json');
    ...
    for (const r of data.reservas ?? []) {
      await client.query(
        ...
        [
          ...
          r.estado ?? 'pendiente', r.notas ?? null,
          r.created_at,
        ],
      );
      reservasCount++;
    ...
    for (const g of data.gastos ?? []) {
      await client.query(
        `INSERT INTO gastos (id, fecha, categoria, subcategoria, descripcion, monto, pagado_por, tipo, created_at)
         ...
         ON CONFLICT (id) DO NOTHING`,
        [
          g.id, g.fecha, g.categoria, g.subcategoria ?? null,
          g.descripcion, g.monto, g.pagado_por, g.tipo, g.created_at,
        ],
      );
      gastosCount++;
    }
    ...
    for (const c of data.consultas ?? []) {
      await client.query(
        `INSERT INTO consultas (id, nombre, adultos, ninos, fecha_inicio, fecha_fin, llegada_tarde, notas, estado, created_at)
         ...
         ON CONFLICT (id) DO NOTHING`,
        [
          c.id, c.nombre, c.adultos, c.ninos,
          c.fecha_inicio, c.fecha_fin, c.llegada_tarde ?? false,
          c.notas ?? null, c.estado, c.created_at,
        ],
      );
      consultasCount++;
    }

    // Reset sequences to avoid ID conflicts on new inserts
